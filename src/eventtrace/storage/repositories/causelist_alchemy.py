...
class SQLAlchemyCauselistRepository:
    ...
    def get_causelist_bench(
        ...
        with Session(self._engine) as session:
            q = (
                ...
                .where(CauselistBench.list_date == list_date, CauselistBench.court_no == court_no)
            )
            if side:
    ...
    def list_causelist_benches(
        ...
        with Session(self._engine) as session:
            q = (
                select(*CauselistBench.__table__.columns, func.count(CauselistCase.id).label("case_count"))
    ...
    def list_causelist_cases(
        ...
        with Session(self._engine) as session:
            ...
            if side or list_type or source_id:
                ...
                if side:
                    sub = sub.where(CauselistBench.side == side)
                if list_type:
    ...
    def search_causelist_cases(
        ...
        with Session(self._engine) as session:
            q = (
                ...
                .join(CauselistBench, CauselistBench.id == CauselistCase.bench_id)
                .order_by(CauselistCase.list_date.desc(), CauselistCase.court_no, CauselistCase.serial_no)
                .limit(limit)
            )
    ...
    def list_causelist_dates(self) -> list[str]:
        with Session(self._engine) as session:
            rows = session.execute(
                select(CauselistBench.list_date).distinct().order_by(CauselistBench.list_date.desc())
            ).scalars().all()
        return list(rows)
...
def _bench_to_dict(r: CauselistBench) -> dict:
    return {
        "id": r.id, "list_date": r.list_date, "court_no": r.court_no,
        "bench_label": r.bench_label, "side": r.side, "list_type": r.list_type,
        "judges_json": r.judges_json, "not_sitting": r.not_sitting, "vc_link": r.vc_link,
        "jurisdiction": r.jurisdiction, "scraped_at": r.scraped_at, "source_id": r.source_id,
        "at_time": r.at_time, "floor": r.floor, "building": r.building,
    }


def _case_to_dict(r: CauselistCase) -> dict:
    return {
        "id": r.id, "bench_id": r.bench_id, "list_date": r.list_date, "court_no": r.court_no,
        "serial_no": r.serial_no, "case_ref": r.case_ref, "case_type": r.case_type,
        "case_number": r.case_number, "case_year": r.case_year, "petitioner": r.petitioner,
        "respondent": r.respondent, "advocate": r.advocate, "pro_se": r.pro_se,
        "ia_numbers_json": r.ia_numbers_json, "section": r.section, "subsection": r.subsection,
        "hearing_type": r.hearing_type, "raw_text": r.raw_text, "scraped_at": r.scraped_at,
    }
